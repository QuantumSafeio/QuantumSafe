/*
  # Enhanced Points System and Ambassador Program

  1. New Tables
    - `points_transactions` - Detailed tracking of all point changes
    - `social_media_stats` - Track social media engagement per platform
    - `ambassador_tiers` - Define ambassador program tiers and benefits
    - `user_achievements` - Track user achievements and milestones

  2. Enhanced Existing Tables
    - `user_points` - Add social_media_points, referral_points, scan_points, ambassador_tier
    - `referrals` - Add commission_rate, total_earned, is_active

  3. Functions
    - `calculate_ambassador_tier()` - Calculate user's ambassador tier
    - `award_points()` - Award points with proper tracking
    - `process_referral_commission()` - Process referral commissions
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND table_name = 'social_media_stats'
  ) THEN
...
-- Create performance indexes with column existence checks
DO $$
BEGIN
  ...
  -- Only create platform index if platform column exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'points_transactions' AND column_name = 'platform'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_points_transactions_platform ON points_transactions(platform);
  END IF;

  -- Create indexes for social_media_stats
...
BEGIN
  ...
  FROM referrals
  WHERE referrer_id = user_uuid AND COALESCE(is_active, true) = true;
...
BEGIN
  ...
  SELECT referrer_id, COALESCE(commission_rate, 7.0) INTO referrer_uuid, commission_rate
  FROM referrals
  WHERE new_user_id = referred_user_uuid AND COALESCE(is_active, true) = true;
