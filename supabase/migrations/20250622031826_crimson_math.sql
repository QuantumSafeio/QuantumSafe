/*
  # Enhanced Points System Migration

  1. New Tables
    - `points_transactions` - Track all point transactions with detailed metadata
    - `social_media_stats` - Track social media engagement and points
    - `ambassador_tiers` - Define ambassador program tiers and benefits
    - `user_achievements` - Track user achievements and milestones

  2. Enhanced Tables
    - Update `user_points` with additional tracking fields
    - Update `referrals` with enhanced commission tracking
...
-- Create policies for social_media_stats
...
-- Create policies for ambassador_tiers (read-only for users)
