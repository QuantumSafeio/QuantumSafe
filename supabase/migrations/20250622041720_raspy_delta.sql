/*
  # Enhanced Points and Ambassador System

  1. New Tables
    - `points_transactions` - Detailed points tracking with source and platform
    - `social_media_stats` - Social media engagement statistics  
    - `ambassador_tiers` - Ambassador program tier definitions
    - `user_achievements` - User achievement tracking

  2. Enhanced Existing Tables
    - `user_points` - Added social_media_points, referral_points, scan_points, ambassador_tier
    - `referrals` - Added commission_rate, total_earned, is_active
  ...
  4. Functions
    ...
    - `calculate_ambassador_tier()` - Calculate user tier based on activity
...
BEGIN
  ...
  INSERT INTO points_transactions (user_id, points_change, source, metadata)
  VALUES (user_uuid, points_amount, point_source, point_metadata);
